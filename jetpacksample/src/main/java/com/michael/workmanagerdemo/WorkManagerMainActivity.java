...
//                .setConstraints(constraints)//设置触发条件
        ...
        //实时监听变化
        WorkManager.getInstance(this).getWorkInfoByIdLiveData(uploadWorkRequest.getId()).observe(
                WorkManagerMainActivity.this, new Observer<WorkInfo>()
        {
            @Override
            public void onChanged(WorkInfo workInfo)
            {
                ...
                if (workInfo != null && workInfo.getState() == WorkInfo.State.SUCCEEDED)
                {
                    ...
                    Toast.makeText(WorkManagerMainActivity.this, "Success!", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    private void startPeriodicWorkRequest()
    {
        ...
        WorkManager.getInstance(this).getWorkInfosByTagLiveData(TAG).observe(
                WorkManagerMainActivity.this, new Observer<List<WorkInfo>>()
        {
            @Override
    ...
    {
        WorkManager.getInstance(WorkManagerMainActivity.this).cancelAllWork();
    }
