 ...
 */
//1.4.1 务实
//务实对我们来说意味着一件简单的事情：Kotlin就是一门设计出来解决现实世
//界问题的实用语言。它的设计基于多年创建大型系统的工业经验，它的特性也是为
//解决许多软件开发者遇到的场景而选择的。此外，来自JetBrains 内部和社区的开发
//者已经使用Kotlin的早期版本很多年，他们的反馈也被融合进了这门语言公开发布
//的版本中。所以我们才能自信地说，Kotlin能够帮助解决实际项目的问题。
...
//此外，Kotlin也没有强制使用某种特定的编程风格和范式。当你开始学习这门
//语言的时候，可以使用熟悉的来自Java 经验的风格。然后，你会渐渐地发现更多强
//大的Kotlin特性，并学习把它们应用到你的代码中，让代码更简洁、更符合语言习惯。
//Kotlin的实用主义的另一个重要体现是对于工具的专注。对开发者的生产力而
//言，一个智能的开发环境和一门设计良好的语言同样重要。因此，事后再来考虑对
//IDE 进行支持就是马后炮。而Kotlin的情况是，IntelliJ IDEA 的插件是和编译器同
//步开发的，并且在设计语言特性时始终牢记着对工具的支持。
//IDE支持对帮助你探索Kotlin的特性也发挥着重要作用。许多情况下，工具会
...
//满足代码结构的需要。许多标准的Java样板代码，例如getter、setter以及将构造方
//法的参数赋值给宇段的逻辑，在Kotlin中都是隐式的，并不会使你的源代码变得
//混乱。
//
//另外一个导致代码变得不必要的冗长的原因是编写显式的代码来完成常见的任
//务，比如定位集合中的元素。和许多其他现代语言一样，Kotlin有丰富的标准库，
//让你用库方法调用来代替这些冗长重复的代码段。Kotlin对lambda 的支持，让小代
//码块可以轻松地传递给库函数。这让你可以把公共的那部分代码全部封装在库中，
//而在用户代码中仅保留特定的针对任务的那部分。
//与此同时，Kotlin并没有尝试把源代码压缩到最小可能的长度。例如，即使
//Kotlin支持运算符重载，用户也不能定义自己的运算符。因此，库开发者不能用神
...
//以及其他错误的动态内存分配造成的问题。作为面向NM 的静态类型语言，Kotlin
//还保证了应用程序的类型安全。这比使用Java 的成本要更低：不需要指定所有的类
//型声明，因为许多情况下编译器会自动地推断出类型。
//Kotlin 所做的不止这些，这意味着更多的原本在运行时失败的错误在编
//译期的检查中就被阻止了。最重要的一点是，Kotlin努力地从你的程序中消除
//NullPointerExceptiono Kotlin的类型系统跟踪那些可以或不可以为null的
//值，并且禁止那些运行时可能导致NullPointerException 的操作。这所带来
//的额外的成本是极小的：把类型标记为可空的只要一个字符，就是类型尾部的一个
//问号：
//
//除此之外，Kotlin提供了许多便利的方法来处理可空数据。这非常有助于消灭
//应用程序的崩溃。
//Kotlin有助于避免的另一种异常类型就是ClassCastExceptiono 当你把一
//个对象转换成一种类型，而没有事先检查它是否是正确的类型时，就会发生这个异
//常。在Java中，开发者常常省略了这类检查，因为必须反复地在检查和其后的转换
//中写明类型名称。另一方面，Kotlin中的检查和转换被组合成了一次操作：一旦检
...
//以在Kotlin中使用它们。可以调用Java的方法，继承Java的类和实现Java 的接口，
//在Kotlin类上应用Java的注解，等等。
//与其他一些NM 语言不同，Kotlin 在互操作性上更上一层楼，让Java 代码也
//可以毫不费力地调用Kotlin的代码。无须取巧：Kotlin 的类和方法可以像常规的
//Java 类和方法一样被调用。这带来了无限的灵活性，在项目的任何地方都可以混合
//使用Java和Kotlin。当你刚开始在自己的Java 项目中引入Kotlin时，可以在代码库
//中的任意一个类上运行Java 到Kotlin 的转换器，剩下的代码不需要任何修改就可以
//继续编译和工作。不管你所转换的类是什么角色，这都是可行的。
//另一个Kotlin专注于互操作性的领域是在最大程度上使用现有的Java库。例如，
//Kotlin 没有自己的集合库，它完全依赖Java 标准库中的类，使用额外的函数来扩展
//它们，让它们在Kotlin中用起来更方便（我们会在3.3 节中了解这种机制更多的细节）。
//这意味着在Kotlin中调用JavaAPI时，永远不需要包装或者转换这些Java对象，反
//之亦然。所有这些Kotlin 提供的丰富的API 在运行时没有任何的额外开销。
//Kotlin工具也对跨语言项目提供了全面支持。它可以编译任意混合的Java和
//Kotlin源码，不管它们之间是怎样互相依赖的。IDE的特性也能跨语言工作，允许：
//· 自由地在Java和Kotlin源码文件之间切换
//· 调试混合语言的项目，可以在不同语言编写的代码之中单步调试
//· 重构Java方法的时候，Kotlin代码中的对它们的调用也会得到正确的更新，反之亦然
//希望我们已经说服你尝试一下Kotlin。现在，你要如何开始使用它？在接下来
//的一节中，我们将从命令行和其他不同工具的使用两方面讨论编译和运行Kotlin代
//码的过程。
