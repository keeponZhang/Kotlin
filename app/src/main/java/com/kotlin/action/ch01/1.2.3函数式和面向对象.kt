...
//让我们可以毫不费力地使用这种风格。例如， Java 8 之前的Java 版本都缺少了这种
//支持。Kotlin 拥有丰富的特性集从一开始就支持函数式编程风格，包括：
//· 函数类型，允许函数接受其他函数作为参数，或者返回其他函数。
//• lambda 表达式，让你用最少的样板代码方便地传递代码块
//· 数据类，提供了创建不可变值对象的简明语法
//· 标准库中包括了丰富的API 集合，让你用函数式编程风格操作对象和集合
//
//Ko ti in 允许你使用函数式编程风格但并没有强制你使用它。当你需要的时候，
//可以使用可变数据，也可以编写带副作用的函数，而且不需要跳过任何多余的步骤。
//然后， 毫无疑问的是，在Kotlin 中使用基于接口和类层次结构的库就像Java 一样简
//单。当编写Ko ti in 代码的时候，可以结合使用面向对象编程和函数式编程风格，并
//使用最合适的工具来对付亟待解决的问题。