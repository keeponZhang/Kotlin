...
//
//
//第一种情况，当你使用集合的时候，列表被变换成了另一个列表，所以map变
//换应用到每一个元素上，包括了数字3和4。然后，第一个满足判断式的元素被找到了：
//数字2 的平方。
//第二种情况，find调用一开始就逐个地处理元素。从原始序列中取一个数字，
//用map变换它，然后再检查它是否满足传给find的判断式。当进行到数字2 时，
//发现它的平方己经比数字3大，就把它作为find操作结果返回了。不再需要继续检
//查数字3和4，因为这之前你己经找到了结果。
//在集合上执行操作的顺序也会影响性能。假设你有一个人的集合，想要打印集
//合中那些长度小于某个限制的人名。你需要做两件事：把每个人映射成他们的名字，
//然后过滤掉其中那些不够短的名字。这种情况可以用任何顺序应用map和filter
//操作。两种顺序得到的结果一样，但它们应该执行的变换总次数是不一样的，如图5 . 9
//所示。
