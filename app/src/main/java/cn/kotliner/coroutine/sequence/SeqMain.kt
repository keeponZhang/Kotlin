...
//    }
    ...
    val var2: Iterator<Int> = fibonacci.iterator()
    //里面是个死循环，如果没有返回值，会一直调用resume，从而走到下面的suspend方法，改变state，执行挂起操作,执行到yield后，会改变状态，var2.hasNext()
    // 返回true（yield启动的时候就会首先调用）
    //这里的逻辑是，调用var2.hasNext()返回true，下面的yield阻塞住的才会被恢复，跳出循环的话，下面的也不会被恢复
    while (var2.hasNext()) {
...
//}
//buildSequence的函数参数是SequenceBuilder类型的，返回Sequence<T>,<T>是yield的参数类型
...
//    我们只需要在while循环体中循环调用yield()来检查该job的取消状态，如果已经被取消，那么isCompleted值将会是true，yield函数就直接抛出CancellationException异常，从而完成取消的功能：
    while (true) {
        //后面需要Continuation.resume回调
        try {
            //yield，会阻塞住，调用者调用var2.hasNext()，才会接着执行下面的代码
